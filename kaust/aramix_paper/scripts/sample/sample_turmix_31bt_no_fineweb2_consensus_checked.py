#!/usr/bin/env python3
"""
Script to sample ~31B tokens from the TurMix MinHash deduped dataset,
filtering FineWeb-2 documents that don't appear in the consensus dataset.

Filtering logic:
- KEEP: Documents from non-FineWeb-2 sources
- KEEP: FineWeb-2 documents that appear in turmix-consensus (have duplicates in other sources)
- DROP: FineWeb-2 documents that don't appear in turmix-consensus (unique to FineWeb-2)

This approach keeps FineWeb-2 content that is corroborated by other sources.

Dataset structure notes:
- minhash_deduped: has `metadata` dict with `source` or `source_0` key
...
SOURCE_MAPPING = {
    ...
    "vngrs": "VNGRS",
    "VNGRS": "VNGRS",
    "vngrs_web_corpus": "VNGRS",
    "VNGRS Web Corpus": "VNGRS",
}
...
def is_fineweb2(source: str) -> bool:
    """Check if source is FineWeb-2."""
    canonical = normalize_source(source)
    return canonical == "FineWeb-2"
...
def estimate_tokens_per_source(
    ...
    Also counts eligible documents (non-FineWeb-2 or FineWeb-2 in consensus).

    Returns:
        avg_tokens_per_doc: Dict mapping source -> average tokens per document
        doc_counts: Dict mapping source -> eligible document count
        total_fineweb2_docs: Total FineWeb-2 documents
        kept_fineweb2_docs: FineWeb-2 documents kept (in consensus)
    """
    ...
    total_fineweb2_docs = 0
    kept_fineweb2_docs = 0
    dropped_fineweb2_docs = 0

    print("Counting documents per source (with FineWeb-2 consensus check)...")
    for pf_path in tqdm(parquet_files, desc="Counting"):
        ...
        if canonical == "FineWeb-2":
            for text in texts:
                total_fineweb2_docs += 1
                # Check if in consensus
                if text_hash(text) in consensus_hashes:
                    kept_fineweb2_docs += 1
                    doc_counts[canonical] += 1
                else:
                    dropped_fineweb2_docs += 1
        else:
            doc_counts[canonical] += len(texts)

    print(f"\nFineWeb-2 statistics:")
    print(f"  Total FineWeb-2 documents: {total_fineweb2_docs:,}")
    if total_fineweb2_docs > 0:
        print(f"  Kept (in consensus): {kept_fineweb2_docs:,} ({100*kept_fineweb2_docs/total_fineweb2_docs:.1f}%)")
        print(f"  Dropped (unique): {dropped_fineweb2_docs:,} ({100*dropped_fineweb2_docs/total_fineweb2_docs:.1f}%)")
    else:
        print("  No FineWeb-2 documents found in dataset")
    ...
    for pf_path in tqdm(parquet_files, desc="Sampling"):
        ...
        # For FineWeb-2, only sample from consensus-approved docs
        if canonical == "FineWeb-2":
    ...
    return avg_tokens_per_doc, dict(doc_counts), total_fineweb2_docs, kept_fineweb2_docs
...
def sample_and_save(
    ...
    FineWeb-2 documents are only included if they appear in consensus.
    ...
    for pf_path in tqdm(parquet_files, desc="Indexing"):
        ...
        for idx, text in enumerate(texts):
            # For FineWeb-2, only include if in consensus
            if canonical == "FineWeb-2":
    ...
    with open(summary_path, "w") as f:
        f.write("TurMix 31B Token Sample Summary (FineWeb-2 Consensus-Checked)\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Total documents: {len(all_sampled_rows):,}\n")
        f.write(f"Estimated total tokens: {estimated_total/1e9:.2f}B\n\n")
        f.write("Filtering: FineWeb-2 docs kept only if they appear in consensus\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Sample ~31B tokens from TurMix dataset (FineWeb-2 consensus-checked)"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
        default="~/data/pretraining_data/turmix-minhash-deduped",
        help="Input dataset directory"
    )
    parser.add_argument(
        "--consensus-dir",
        type=str,
        default="~/data/pretraining_data/turmix-consensus",
        help="Consensus dataset directory (for FineWeb-2 checking)"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="~/data/turmix-minhash-no_fineweb2_consensus_checked-31bt",
    ...
    print("=" * 70)
    print("TurMix 31B Token Sampler (FineWeb-2 Consensus-Checked)")
    print("=" * 70)
    ...
    print("=" * 70)
    print("\nFiltering logic:")
    print("  - KEEP: Non-FineWeb-2 documents")
    print("  - KEEP: FineWeb-2 documents that appear in consensus")
    print("  - DROP: FineWeb-2 documents NOT in consensus (unique to FineWeb-2)")
    print("=" * 70)
    ...
    # Estimate tokens per source
    avg_tokens, doc_counts, total_fineweb2, kept_fineweb2 = estimate_tokens_per_source(
        input_dir,
        consensus_hashes,
        tokenizer,
