#!/usr/bin/env python3
"""
Script to sample ~30B tokens from the HinMix Consensus dataset
while filtering out documents that only have CulturaX + one other source.

Filtering rules:
- DROP: Documents with exactly 2 sources where one is 'culturax'
- KEEP: Documents with 'culturax' + 2 or more other sources (3+ total)
- KEEP: Documents without 'culturax'
"""
...
# CulturaX identifier (lowercase as seen in data)
CULTURAX = "culturax"
...
def should_keep_document(sources: List[str]) -> bool:
    ...
    Rules:
    - DROP: Exactly 2 sources where one is 'culturax'
    - KEEP: 'culturax' + 2 or more other sources (3+ total)
    - KEEP: No 'culturax' in sources
    """
    if CULTURAX not in sources:
        # No culturax -> keep
        return True

    # Has culturax
    if len(sources) >= 3:
        # 3+ sources means 2+ non-culturax sources -> keep
        return True

    # Has culturax and only 2 sources total -> drop
    return False
...
def sample_and_save(
    ...
    with open(summary_path, "w") as f:
        f.write("HinMix Consensus 30B Token Sample Summary (No CulturaX-only pairs)\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Total documents: {len(all_sampled_rows):,}\n")
        f.write(f"Estimated total tokens: {estimated_total/1e9:.2f}B\n\n")
        f.write("Filtering rule: Dropped docs with exactly 2 sources where one is 'culturax'\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Sample ~30B tokens from HinMix Consensus dataset (filtering CulturaX-only pairs)"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
        default="~/data/aramix_paper/hinmix-consensus",
        help="Input dataset directory"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="~/data/aramix_paper/hinmix-consensus-no_culturax-30bt",
    ...
    print("=" * 70)
    print("HinMix Consensus 30B Token Sampler (No CulturaX-only pairs)")
    print("=" * 70)
    ...
    print("=" * 70)
    print("\nFiltering rule:")
    print("  - DROP: Documents with exactly 2 sources where one is 'culturax'")
    print("  - KEEP: Documents with 'culturax' + 2+ other sources")
    print("  - KEEP: Documents without 'culturax'")
    print("=" * 70)
