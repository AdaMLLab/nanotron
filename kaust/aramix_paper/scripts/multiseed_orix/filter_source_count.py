"""Filter parquet files by `len(source) >= threshold` (list-length proxy for duplication).

Usage:
    python filter_source_count.py <src_dir> <dst_dir> <threshold>
...
def main(src_dir: str, dst_dir: str, threshold: int) -> None:
    ...
    for f in files:
        ...
        lens = pc.list_value_length(table["source"])
        mask = pc.greater_equal(lens, threshold)
