...
#SBATCH --output=/mnt/data/u/alrashsm/nanotron_checkpoints/logs/hplt2_indonesian_seed42_%j.out
#SBATCH --error=/mnt/data/u/alrashsm/nanotron_checkpoints/logs/hplt2_indonesian_seed42_%j.err
...
_CKPT_DIR="/mnt/data/u/alrashsm/nanotron_checkpoints/hplt2_indonesian_seed42"
_TRAIN_STEPS=14000
_SELF_SCRIPT="/home/alrashsm/github/nanotron/kaust/aramix_paper/scripts/multiseed_orix/run_hplt2_indonesian_seed42.sh"
...
CONFIG_PATH="${NANOTRON_DIR}/kaust/aramix_paper/configs/multiseed_indonesian/config_llama_1.46B_hplt2_indonesian_seed42.yaml"

DATA_BASE="/mnt/data/u/alrashsm"
HPLT_ROOT="${DATA_BASE}/datasets/hplt2-indonesian"
HPLT_FLAT="${DATA_BASE}/datasets/hplt2-indonesian/flat"

mkdir -p "${DATA_BASE}/nanotron_checkpoints/hplt2_indonesian_seed42"
mkdir -p "${DATA_BASE}/nanotron_checkpoints/logs"

export CUDA_DEVICE_MAX_CONNECTIONS=1
export OMP_NUM_THREADS=4

echo "=== Training: hplt2-indonesian seed42 ==="
echo "    Config: ${CONFIG_PATH}"
echo "    Node: $(hostname)"
echo "    Job ID: ${SLURM_JOB_ID}"
echo "    Conda env: ${CONDA_DEFAULT_ENV}"

# Flatten the 4 HPLT2 shards (ind_Latn_1..ind_Latn_4) into one flat dir via symlinks.
# Idempotent: existing symlinks are preserved.
echo ""
echo "=== Flatten HPLT2 shards into single dir ==="
mkdir -p "${HPLT_FLAT}"
COUNT=0
for shard in "${HPLT_ROOT}"/ind_Latn_*/; do
    [ -d "${shard}" ] || continue
    SHARD_NAME=$(basename "${shard}")
    for f in "${shard}"*.parquet; do
        [ -f "${f}" ] || continue
        LINK="${HPLT_FLAT}/${SHARD_NAME}_$(basename "${f}")"
        if [ ! -e "${LINK}" ]; then
            ln -s "${f}" "${LINK}"
            COUNT=$((COUNT+1))
        fi
...
echo "  Created ${COUNT} new symlinks ($(ls "${HPLT_FLAT}" | wc -l) total)"

source "${SCRIPTS_DIR}/tokenize_helper.sh"
tokenize_dataset \
    "${HPLT_FLAT}" \
    "${DATA_BASE}/nanotron_checkpoints/tokenized_data/hplt2-indonesian" \
    "hplt2-indonesian" "hf" "text"
