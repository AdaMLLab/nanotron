...
#SBATCH --requeue
#SBATCH --output=/mnt/data/u/alrashsm/nanotron_checkpoints/logs/consensus_dup4_%j.out
#SBATCH --error=/mnt/data/u/alrashsm/nanotron_checkpoints/logs/consensus_dup4_%j.err
...
CONFIG_PATH="${NANOTRON_DIR}/kaust/aramix_paper/configs/multiseed_aramix/config_llama_1.46B_consensus_dup4.yaml"

DATA_BASE="/mnt/data/u/alrashsm"
RAW_SRC="${DATA_BASE}/datasets/aramix-matched/data/consensus"
FILTERED_DIR="${DATA_BASE}/datasets/aramix-matched-4/data/consensus"
TOKENIZED_DIR="${DATA_BASE}/nanotron_checkpoints/tokenized_data/aramix-consensus-dup4"

mkdir -p "${DATA_BASE}/nanotron_checkpoints/consensus_dup4"
mkdir -p "${DATA_BASE}/nanotron_checkpoints/logs"

export CUDA_DEVICE_MAX_CONNECTIONS=1
export OMP_NUM_THREADS=4

echo "=== Training: consensus duplicated>=4 ==="
echo "    Config: ${CONFIG_PATH}"
echo "    Node: $(hostname)"
echo "    Job ID: ${SLURM_JOB_ID}"
echo "    Conda env: ${CONDA_DEFAULT_ENV}"

# --- Step 1: Filter parquet files (duplicated >= 4) ---
echo ""
echo "=== Filter step ==="
if ls "${FILTERED_DIR}"/train-*.parquet 1>/dev/null 2>&1; then
    echo "[filter] Already filtered at ${FILTERED_DIR}, skipping."
else
    python "${SCRIPTS_DIR}/filter_parquet.py" "${RAW_SRC}" "${FILTERED_DIR}" 4
fi
...
if ls "${TOKENIZED_DIR}"/*.ds 1>/dev/null 2>&1; then
    ...
    python3 tools/preprocess_data.py \
        ...
        --n-tasks 32 \
        hf \
        --dataset "${DATA_BASE}/datasets/aramix-matched-4/data/consensus" \
        --column text \
        --split train
fi
